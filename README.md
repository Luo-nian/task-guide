...
- ✅ **4 分类**：每日任务 / 目标任务 / 限时任务 / **次数任务**（重命名，原"小任务"）
- ⚡ **紧急任务提醒**：≤48h 任务按 3/10 提醒；>48h 任务剩 36h 提醒
- 🏆 **积分 + 等级系统**：集满积分升级（历练学徒→风华游侠→云霜骑士→群星行者→传奇勇者），每级对应努力角色
- 📈 **每日进度条**（3 样式可切换）：条形 / 圆形 / 点状；次数任务超额完成时进度条凸出
- 🌊 **追踪涟漪**：追踪中任务用蓝色小菱形 + 呼吸金色涟漪边框
- 📦 **历史任务**（重命名，原"仓库"）：已完成任务放侧边栏底部，按时间从新到旧 + 检索
- 🎉 **完成祝福**：完成当日最后一项任务时弹出原神风祝福卡
- 🕒 **22:00 弹窗**：每日任务到晚上 10 点还没完成时提醒
- 🧩 **步骤推进**：详情页可直接勾选完成步骤，实时更新进度
